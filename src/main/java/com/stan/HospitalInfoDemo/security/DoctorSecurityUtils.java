...
public class DoctorSecurityUtils {
	...
	public static void sendAuthenticationSuccessResponse(HttpServletResponse httpServletResponse, int status, String message, Exception exception, DoctorUser doctorUser)
			throws IOException {
		Response response = new DoctorAuthenticationSuccessResponse(exception == null ? true : false, status, message, doctorUser);
	...
	public static boolean isDoctor(Collection<? extends GrantedAuthority> profiles) {
		boolean isDoctor = false;
		for(GrantedAuthority profle: profiles) {
			if(profle.getAuthority().equals("ROLE_DOCTOR")) {
				isDoctor = true;
			}
		};
		return isDoctor;
	...
	public static boolean isNurse(Collection<? extends GrantedAuthority> profiles) {
		boolean isNurse = false;
		for(GrantedAuthority profle: profiles) {
			if(profle.getAuthority().equals("ROLE_NURSE")) {
				isNurse = true;
			}
		};
		return isNurse;
